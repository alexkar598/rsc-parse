...
import { MaybeEmptyRscEntry } from "./rsc_entry.js";
...
function encodeEntry(buf: DataView, entry: MaybeEmptyRscEntry) {
  ...
  if (!entry.used) {
    ...
    arr.set(entry.content, pos);
    pos += entry.content.byteLength;
    arr.set(entry.padding, pos);
    pos += entry.padding.byteLength;
  }
